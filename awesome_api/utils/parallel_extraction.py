...
def multiple_dummy_func(values: Sequence[str], chunk_size:int, pool_size: int) -> DataFrame:
    return parallel_execution(func=dummy_func, values=values, chunk_size=chunk_size, pool_size=pool_size)

if __name__ == '__main__':
    print(
        multiple_dummy_func(
            values=[f"a_{i}" for i in range(1, 51256)],
            chunk_size=1000,
            pool_size=6
        ).tail(10)
    )
